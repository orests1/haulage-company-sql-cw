...
accept my_return char prompt 'Enter end month number : '
-- Get the first 10 top pickups 
SELECT *
FROM (
	-- Get the unique pickups list
    SELECT cnt1.company_name,COUNT(cnt1.company_name) AS Pickups
    FROM (
	-- Get all the pickups list
	SELECT COUNT(*) as total, c.company_name
